...
interface UserDocumentFloatingButtonProps {
  currentPage: number;
  totalPages: number;
  onPreviousPage: () => void;
  onNextPage: () => void;
...
export default function UserDocumentFloatingButton({
  currentPage,
  ...
  isPreviousDisabled,
  isNextDisabled
}: UserDocumentFloatingButtonProps) {
  ...
  useEffect(() => {
    ...
    const savedPosition = localStorage.getItem('user-document-floating-button-position');
    if (savedPosition) {
      try {
  ...
  const savePosition = useCallback((pos: Position) => {
    if (isClient) {
      localStorage.setItem('user-document-floating-button-position', JSON.stringify(pos));
    }
  }, [isClient]);
  ...
  const constrainPosition = useCallback((pos: Position): Position => {
    ...
    const isMobile = window.innerWidth < 768;
    const margin = isMobile ? 15 : 20; // 移动端使用更小的边距
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center space-y-3 sm:space-y-3 lg:space-y-3.5 animate-float">
        {/* 上一页按钮 */}
        <button
          onClick={onPreviousPage}
          disabled={isPreviousDisabled}
          className="group relative w-12 h-12 sm:w-11 sm:h-11 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center text-slate-600 hover:text-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-300 ease-out hover:scale-105 active:scale-95 overflow-hidden"
        ...
        >
          ...
          <svg className="relative w-5 h-5 sm:w-4.5 sm:h-4.5 lg:w-5 lg:h-5 transition-all duration-300 group-hover:-translate-y-0.5 group-hover:text-slate-800 drop-shadow-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
        ...
        <div
          ref={numberButtonRef}
          className="relative w-12 h-7 sm:w-11 sm:h-7 lg:w-12 lg:h-8 rounded-2xl flex items-center justify-center transition-all duration-300 hover:scale-105 cursor-move overflow-hidden"
        ...
        >
          ...
          <div className="absolute inset-0 rounded-2xl opacity-0 hover:opacity-100 transition-opacity duration-300" 
            ...
            <span className="relative text-sm sm:text-xs lg:text-sm font-bold text-white tracking-wide drop-shadow-sm">
              {currentPage}
            </span>
        </div>
